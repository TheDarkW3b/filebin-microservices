...
function ViewUploadedFiles() {
  ...
  const fetchFiles = async (page) => {
    try {
      const response = await axios.get(
        `/api/v1/file/get-files?page=${page - 1}`,
        {
          headers: {
  ...
  const applyFilters = (filesToFilter) => {
    const filtered = filesToFilter.filter((item) => {
  ...
  const handleDeleteFile = (deletedFileId) =>
    setFilteredItems((prevFilteredItems) =>
      prevFilteredItems.filter((file) => file.id !== deletedFileId)
    );
  const handleLikedFile = (likedFileId, likedValue) => {
    setFilteredItems((prevFilteredItems) =>
      prevFilteredItems.map((file) =>
        file.id === likedFileId ? { ...file, liked: likedValue } : file
      )
    );
  };
      ...
      {filteredItems.length === 0 ? (
        <div className="flex flex-col justify-center items-center mt-8 h-full fade-in">
          ...
          <p className="mt-2 text-gray-500 md:text-gray-400">
            You can upload a new file to get started!
          </p>
        </div>
      ) : (
