...
function ViewLiked() {
  ...
  const fetchLiked = async (page) => {
    try {
      const response = await axios.get(
        `/api/v1/user/get-liked?page=${page - 1}`,
        {
          headers: {
      ...
      if (response.status === 200) {
        setLiked(response.data.content);
  ...
  const applyFilters = (likesToFilter) => {
    const filtered = likesToFilter.filter((item) => {
      const itemType = item.contentType.split("/")[1];
  ...
  const handleDeleteLiked = (deletedItemId) => {
    setFilteredItems((prevFilteredItems) =>
      prevFilteredItems.filter((item) => item.id !== deletedItemId)
    );
  };

  const handleFavoriteLiked = (likedItemId, likedValue) => {
    setFilteredItems((prevFilteredItems) =>
      prevFilteredItems.map((item) =>
        item.id === likedItemId ? { ...item, liked: likedValue } : item
      )
    );
  };
    ...
    <div className="mx-auto w-full">
      <div className="flex md:flex-row flex-col justify-between gap-4 md:gap-0 mb-6">
        ...
        <div className="flex items-center gap-4">
          ...
          {/* Subject Filter (for paste titles) with icon */}
          <div className="relative flex items-center w-full max-w-xs">
            <MdOutlineFilterAlt className="left-2 absolute w-5 h-5 text-gray-500 dark:text-white" />
            <select
              name="subject"
      ...
      {filteredItems.length === 0 ? (
        <div className="flex flex-col justify-center items-center mt-8 h-full fade-in">
          ...
          <p className="mt-2 text-gray-500 md:text-gray-400">
            You can like any file or paste to get started!
          </p>
        </div>
      ) : (
          ...
          <div key={index} className="mb-4">
            {item.pasteName ? (
              <PasteCard
                key={index}
                pasteId={item.id}
                pasteName={item.pasteName}
                uploadDate={item.uploadTime}
                pasteSize={item.size}
                pasteType={item.contentType}
                description={item.description}
                onDelete={handleDeleteLiked}
                isLiked={item.liked}
                onLike={handleFavoriteLiked}
              />
            ) : (
              <FileCard
                key={index}
